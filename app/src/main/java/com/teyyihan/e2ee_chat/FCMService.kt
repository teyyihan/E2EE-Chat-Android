...
class FCMService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(p0: RemoteMessage?) {
        ...
        runBlocking {
            getMessageModel(p0?.data)?.let {
                val myPrivKey = sessionManager.getUserCache()?.privateKey
                val agree = KeyUtil.keyAgreement(myPrivKey!!,it.senderPublicKey)
                val aes = KeyUtil.generateSharedAESKey(agree)
                val decrypted = KeyUtil.encryptOrDecrypt(it.cipher,aes, Cipher.DECRYPT_MODE,it.messageSpecificIV)
                Log.d(TAG, "onMessageReceived: cipher: ${String(it.cipher)}  decrypted: ${String(decrypted)}")
            }
        }
...
//                val insertedFriend = insertFriendIntoDB(it,friendLocalDataSource)
...
//
    private fun getMessageModel(data: Map<String, String>?): FCMMessageModel? {
        try {
            ...
            val cipherText = extractMessageBytes(data?.get("cipherText"))
            val date = data?.get("date")
            val messageSpecificIV = extractMessageBytes(data?.get("messageSpecificIV"))
            val senderPublicKey = data?.get("senderPublicKey")?.convertToPublicKey()
